...
class SupplierController extends Controller
{
     ...
     */
    public function getSuppliersAction(ParamFetcherInterface $paramFetcher)
    {
        ...
        $query->select(array('p'))
            ->from('AppClientBundle:Supplier', 'p')
        ;
     ...
     *
     */
    public function getSupplierAction($article, Request $request)
    {
        $query = explode('/', $request->getUri());
        $id = end($query);
        $data = $this->getDoctrine()->getRepository('AppClientBundle:Supplier')->findOneBy(['id' => $id]);
    ...
    {
        return $this->createForm(new \App\SupplierBundle\Form\SupplierType(), $article);
    }
     ...
     * @Put("/suppliers/{id}")
     */
    public function putSuppliersAction(Request $request, $id)
    {
        $em = $this->getDoctrine()->getEntityManager();
        ...
        $initial = $this->getDoctrine()->getRepository('AppClientBundle:Supplier')->findOneBy(['id' => $id]);
        if(!is_null($initial))
        {
     ...
     * @Delete("/suppliers/{id}")
     */
    public function deleteSupplierAction($id)
    {
        $em = $this->getDoctrine()->getEntityManager();

        $supplier = $this->getDoctrine()->getRepository('AppClientBundle:Supplier')->findOneBy(['id' => $id]);
