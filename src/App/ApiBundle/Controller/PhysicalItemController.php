...
class PhysicalItemController extends Controller
{
     ...
     */
    public function getPhysicalItemsAction(ParamFetcherInterface $paramFetcher)
    {
        ...
        $query->select(array('p'))
            ->from('AppClientBundle:PhysicalItem', 'p')
        ;
     ...
     *
     */
    public function getPhysicalItemAction($article, Request $request)
    {
        $query = explode('/', $request->getUri());
        $id = end($query);
        $data = $this->getDoctrine()->getRepository('AppClientBundle:PhysicalItem')->findOneBy(['id' => $id]);
    ...
    {
        return $this->createForm(new \App\ClientBundle\Form\PhysicalItemType(), $article);
    }
     ...
     *
     */
    public function postPhysicalItemsAction(Request $request)
    {
        $em = $this->getDoctrine()->getEntityManager();
        $form = $this->getForm();
     ...
     * @Put("/items/{id}")
     */
    public function putPhysicalItemsAction(Request $request, $id)
    {
        $em = $this->getDoctrine()->getEntityManager();
        ...
        $initial = $this->getDoctrine()->getRepository('AppClientBundle:PhysicalItem')->findOneBy(['id' => $id]);
        if(!is_null($initial))
        {
     ...
     * @Delete("/items/{id}")
     */
    public function deletePhysicalItemAction($id)
    {
        $em = $this->getDoctrine()->getEntityManager();

        $item = $this->getDoctrine()->getRepository('AppClientBundle:PhysicalItem')->findOneBy(['id' => $id]);
